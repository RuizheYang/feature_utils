...
class BinStat:
    ...
    def bin_stat(df, y, x, cut_list=None, **kwargs):
        ...
        #
        # # 判断badprob和分箱的单调关系，整体上由坏到好进行排列。
        # res_tmp = res.dropna(subset=['badprob'])
        # try:
        #     sort_type = np.polyfit(x=res_tmp.index.tolist(), y=res_tmp.badprob.tolist(), deg=1)[0] < 0
        #     res.sort_index(ascending=sort_type, inplace=True)
        ...
        cols = 'variable,bin,count,count_distr,bad,badprob,lift,iv_total'.split(',')
        return res[cols]
